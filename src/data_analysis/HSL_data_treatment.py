...
encounter["target_internacao"] = (encounter["de_tipo_atendimento"] == "internado").astype(int)

encounter = encounter[
    ["id_paciente", "id_atendimento", "target_internacao", "TI"]
]
...
patient["age"] = dt.datetime.now().year - pd.to_numeric(patient["aa_nascimento"], errors="coerce")
...
results = results[["ID_PACIENTE", "ID_ATENDIMENTO", "DT_COLETA", "PARA_ANALITO", "DE_RESULTADO"]]
results["DE_RESULTADO"] = pd.to_numeric(results["DE_RESULTADO"].str.replace(",", "."), errors="coerce")
...
final = final.merge(results, left_on=["id_paciente", "id_atendimento"], right_on=["ID_PACIENTE", "ID_ATENDIMENTO"],
                    how="left")
final.drop(["ID_PACIENTE", "ID_ATENDIMENTO"], axis=1, inplace=True)
final.sort_values(["id_atendimento", "DT_COLETA"], inplace=True)

final = final.drop("id_paciente", axis=1).groupby("id_atendimento").first().drop("DT_COLETA", axis=1)
